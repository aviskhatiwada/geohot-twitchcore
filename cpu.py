...
'''
lui (upper 20) and addi (lower 12) sequence is used to move 32-bit immediate values to registers
'''
...
if __name__ == "__main__":
  ...
  if not os.path.isdir('test-cache'):
    os.mkdir('test-cache')
  for x in glob.glob("../twitchcore/riscv-tests/isa/rv32ui-p-*"):
    if x.endswith('.dump'):
      continue
    with open(x, 'rb') as f:
      reset()
      print("test", x)
      e = ELFFile(f)
      it_ = itertools.islice(e.iter_segments(), 1, None)
      for s in it_:
